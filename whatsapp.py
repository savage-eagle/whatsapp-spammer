...
class WhatsApp:
	...
	def __init__(self, wait, screenshot=None, session=None):
		...
		if session:
			...
			self.browser = webdriver.Chrome(executable_path=r"./chromedriver.exe", options=chrome_options)  # we are using chrome as our webbrowser
		else:
		...
		self.search_selector = "._1JAUF > div:nth-child(2)"
	...
	# to get user name from the conversation
	def get_user_chat_name(self):
		try:
			chat_name = self.browser.find_element_by_css_selector("._2KQyF").text
			return chat_name
		except TimeoutException:
			raise TimeoutError("Your request has been timed out! Try overriding timeout!")
		except NoSuchElementException:
	...
	def clear_chat(self, name):
		self.browser.find_element_by_css_selector("._3QfZd").send_keys(name+Keys.ENTER)
	...
	def get_invite_link_for_group(self, groupname):
		search = self.browser.find_element_by_css_selector("._3QfZd")
